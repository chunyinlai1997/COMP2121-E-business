...
if(isset($_POST['number'])){
	$sqlcart = mysql_query("SELECT product_id FROM shopping_cart WHERE m_id = '$m_id' ");
	$output = mysql_num_rows($sqlcart)." <i class='fa fa-shopping-cart'></i>";
	if($output==0){
		$output = "0 <i class='fa fa-shopping-cart'></i>";
	}
	echo $output;
}
...
if(isset($_POST['cart'])){
	...
	$output .="	</ul><hr>";
	$output .="<h5>Grand Total: $$amount HKD</h5><a href='checkout' role='button' class='btn btn-success'>Checkout</a>";
...
if(isset($_POST['delete'])){
	$pid = $_POST['delete'];
	mysql_query("DELETE FROM shopping_cart WHERE product_id = '$pid' ");

}

if(isset($_POST['add'])){
	$check = mysql_query("SELECT COUNT(*) FROM shopping_cart WHERE product_id = '$pid' AND m_id = '$m_id' ");
	...
	if($row[0]==1){
		...
		$pid = $_POST['add'];
		mysql_query("INSERT INTO shopping_cart(product_id,m_id) VALUES('$pid','$m_id')");
		$msg = "ok";
	}
	echo $msg;
}
...
function remove_outdated(){
	...
	{
		$pid = $row[0];
		$sql = mysql_query("SELECT STATUS FROM product WHERE product_id = '$pid' ");
		$row2 = mysql_fetch_array($sql,MYSQL_NUM);
